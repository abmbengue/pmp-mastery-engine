# Content Expansion (Phase 10)

## Goal

Grow PLA into a rich **micro-learning** platform without changing core architecture.

## Academies enriched

| Academy | Approx. lessons | Modules |
|---|---|---|
| Personal Finance | 29 | foundations, debt, saving-investing, wealth-building |
| Corporate Finance | 30 | financial-statements, working-capital, valuation, dcf, ma-basics |
| PMP (original) | 38 | foundations, people, process, business-environment, agile, hybrid, situational-thinking |

Catalogs live under `prisma/seed/content/`.

## Lesson quality rules

Each lesson includes FR/EN text, exercise, quiz, flashcard, skill, difficulty, duration, LEARN→MASTER phases.

Financial content is **educational only** (not personalized advice).  
PMP content is **original** — no PMBOK / PMI protected reproduction.

## Related docs

- [SHORTS.md](./SHORTS.md)
- [LEARNING_PATHS.md](./LEARNING_PATHS.md)
- [SPACED_REPETITION.md](./SPACED_REPETITION.md)
- [CONTENT_MODEL.md](./CONTENT_MODEL.md)
